...
- **MaterialXDocument**: Represents the root document with attributes and top-level elements
...
1. **`.json`** - Human-readable JSON representation
2. **`.mxpb`** - Binary Protocol Buffer format (compact serialization)
3. **`_converted.mtlx`** - Round-trip conversion back to MaterialX XML (for validation)
...
This will generate a MaterialX XML file named `document__from_pb.mtlx`.
...
- Traverses the MaterialX document tree
- Extracts element names, types, and attributes
- Recursively converts child elements
- Preserves document-level attributes

#### ProtobufToMaterialX
Converts Protocol Buffer format back to MaterialX:
- Reconstructs MaterialX document structure
- Restores element hierarchy and attributes
- Creates valid MaterialX documents

#### Util
Utility functions for debugging and visualization:
- `debug_inspect()` - Detailed element tree inspection
- `debug_inspect_compact()` - Compact tree view
- `debug_inspect_simple()` - Simple tree visualization
- `generate_mermaid_diagram()` - Generate Mermaid diagrams for visualization

### main.py

Command-line interface that:
1. Reads MaterialX XML document
2. Converts to Protocol Buffer format
3. Serializes to JSON and binary formats
4. Performs round-trip conversion validation
5. Optionally generates Mermaid diagrams

## Sample Files

The repository includes sample MaterialX documents:
- `standard_surface_chess_set.mtlx` - Chess set with standard surface shader
- `unlit_cross.mtlx` - Simple unlit cross geometry

Each sample includes its converted outputs (`.json`, `.mxpb`, `_converted.mtlx`).

## Visualization

Enable Mermaid diagram generation in `main.py` by setting `write_mermaid = True`. This generates a `.md` file with a Mermaid diagram showing the document structure, which can be viewed in Markdown preview or at [https://mermaid.live](https://mermaid.live).

## License

This is a prototype project for MaterialX serialization research.
